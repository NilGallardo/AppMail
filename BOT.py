...
def resource_path(relative_path):
    """Obtiene la ruta correcta para assets, funcionando en .exe y en script"""
    try:
...
def seleccionarExcel():
    ...
    if archivo:
        ...
        cargar_proveedores()  # <== cargamos proveedores al seleccionar
...
def cargar_proveedores():
    ...
    try:
        ...
        for _, row in agrupado.iterrows():
            ...
            proveedor_frame = customtkinter.CTkFrame(frameChecks,width=20,height=20)
            proveedor_frame.pack(anchor="w", padx=5, pady=5, fill="x",expand=True)

            prov_var = tk.BooleanVar(value=True)
            prov_chk = customtkinter.CTkCheckBox(proveedor_frame, text=proveedor, variable=prov_var,
                                                checkbox_height=20, checkbox_width=20, corner_radius=50)
            prov_chk.grid(row=0, column=0, sticky="w",padx=25)

            toggle_btn = customtkinter.CTkButton(proveedor_frame, text=">", width=20, height=20, font=("Arial", 12))
            toggle_btn.grid(row=0, column=0, padx=0,sticky="w")

            pedidos_frame = customtkinter.CTkFrame(proveedor_frame, width=20, height=20)
            pedidos_frame.grid(row=1, column=0, columnspan=2, sticky="w", padx=20, pady=2)
            pedidos_frame.grid_remove()

            pedido_vars = []
            # Flag para evitar loops infinitos
            internal_change = {"flag": False}
            for pedido in pedidos:
                var = tk.BooleanVar(value=True)
                if not any(p == pedido for p, _ in pedido_vars):
                    chk = customtkinter.CTkCheckBox(pedidos_frame, text=f"Pedido {pedido}", variable=var,
                                                    checkbox_width=18, checkbox_height=18, corner_radius=50)
                    chk.pack(anchor="w", padx=5, pady=1)
                    
                    pedido_vars.append((pedido, var))
                
                # Cuando se cambia un pedido, si está marcado, marcar proveedor
                def on_pedido_change(var=var, prov_var=prov_var, pedido_vars=pedido_vars):
                    if internal_change["flag"]:
                        return
                    internal_change["flag"] = True
                    if var.get():
                        # Si se marca un pedido, marca el proveedor
                        prov_var.set(True)
                    else:
                        # Si todos los pedidos están desmarcados, desmarca el proveedor
                        if not any(p_var.get() for _, p_var in pedido_vars):
                            prov_var.set(False)
                    internal_change["flag"] = False

                var.trace_add("write", lambda *args, v=var, p=prov_var, pv=pedido_vars: on_pedido_change(v, p, pv))
            # Cuando se cambia proveedor, marcar/desmarcar todos los pedidos
            def on_proveedor_change(*args, prov_var=prov_var, pedido_vars=pedido_vars):
                if internal_change["flag"]:
                    return
                estado = prov_var.get()
                internal_change["flag"] = True
                for _, p_var in pedido_vars:
                    p_var.set(estado)
                internal_change["flag"] = False
            prov_var.trace_add("write", on_proveedor_change)

            def toggle(frame=pedidos_frame, btn=toggle_btn):
            ...
            checkboxes.append((correo, prov_var, pedido_vars))
            
            def selectAll():
                for _,prov_var,_ in checkboxes:
                    prov_var.set(True)
...
def get_app_path():
    # Si estamos en un .exe generado por PyInstaller:
    if hasattr(sys, "_MEIPASS"):
        return os.path.dirname(sys.executable)
    # Si estamos en modo script normal:
...
def seleccionarFirma():
    ...
    try:
        ...
        carpeta_firmas = os.path.join(base_path, "Firmas")
        os.makedirs(carpeta_firmas, exist_ok=True)
        nombre_archivo = os.path.basename(archivo)
        destino = os.path.join(carpeta_firmas, nombre_archivo)
        # Copiar solo si no es el mismo archivo (para evitar copiar en bucle)
...
def enviar_correos():
    ...
    try:
        ...
        for correo, prov_var, pedidos_list in checkboxes:
            if not prov_var.get():
                continue
            pedidos_a_enviar = [
                str(p) for p, var in pedidos_list if var.get()
            ]
...
btnExcel = customtkinter.CTkButton(ventana,text="Seleccionar Excel",command=seleccionarExcel,corner_radius=50,fg_color="transparent",border_width=2,border_color="black")
...
btnFirma = customtkinter.CTkButton(ventana,text="Cargar firma desde archivo",command=seleccionarFirma,corner_radius=50,fg_color="transparent",border_width=2,border_color="black")
btnFirma.grid(row=1, column=2,padx=10, pady=20)

btnEnviar = customtkinter.CTkButton(ventana,text="Enviar Mail",command=enviar_correos,corner_radius=50,fg_color="transparent",border_width=2,border_color="black")
